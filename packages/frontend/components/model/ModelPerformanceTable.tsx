...
import { ExternalLink, ArrowUp, ArrowDown, ChevronDown, ChevronUp } from "lucide-react";
import ProviderLogo from "@/components/ui/ProviderLogo";
import { getInputLengthBucketLabel } from "@/lib/inputLength";
import { getSpeedRating, formatNumber } from "@/lib/utils";
...
function RankBadge({ rank }: { rank: number }) {
  ...
  if (rank === 3) {
    return <span className="text-lg font-bold text-theme-accent-warning">3</span>;
  }
  return (
...
export default function ModelPerformanceTable({
  ...
  const displayData = expanded ? data : data.slice(0, DEFAULT_VISIBLE_ROWS);
  const hasMore = data.length > DEFAULT_VISIBLE_ROWS;
        ...
        <table className="w-full min-w-[700px] border-collapse">
          ...
          <tbody>
            {displayData.map((entry, index) => {
              const rank = index + 1;
              const rating = getSpeedRating(entry.avg_tps);
              ...
              return (
                <tr
                  key={`${entry.provider}-${entry.input_length_bucket ?? "all"}-${index}`}
                ...
                >
                  ...
                  <td className="py-3">
                    <div className="flex items-center gap-2.5">
                      <ProviderLogo provider={entry.provider} size={24} />
                  ...
                  <td className="py-3">
                    <span className="text-sm text-theme-text-muted">
                      {entry.input_length_bucket === null
                        ? "平均"
                        : getInputLengthBucketLabel(entry.input_length_bucket)}
                    </span>
                  </td>
                  ...
                  <td className={`py-3 text-right ${isSortedSamples ? "text-theme-accent-violet" : ""}`}>
                    <span className="inline-flex items-center gap-1.5">
                      ...
                      >
                        {formatNumber(entry.sample_count)}
                      </span>
                      <DataQualityDot quality={entry.sample_count > 100 ? "sufficient" : "limited"} />
                    </span>
                  </td>
                  ...
                  <td className="py-3 pr-4 text-right">
                    <Link
                      href={`/provider/${entry.provider}/model/${encodeURIComponent(model)}`}
